...
function Stat({ label, value, sub }) {
  return (
    <div className="p-4 text-center rounded-2xl bg-slate-900/30 border border-white/10">
...
function ValueCard({ icon:Icon, title, points, accent="emerald" }) {
  const tone = accent === 'orange' ? 'text-orange-300 border-orange-400/50' : 'text-emerald-300 border-emerald-400/50'
  return (
    <div className={`rounded-2xl bg-transparent p-5 border-l-2 ${tone} `}>
      <div className={`inline-flex items-center gap-2 text-sm mb-2 ${accent === 'orange' ? 'text-orange-300' : 'text-emerald-300'}`}><Icon className="w-4 h-4"/> {title}</div>
      <ul className="space-y-2 text-white/80 text-sm">
        {points.map((p,i)=> (
          <li key={i} className="flex items-start gap-2"><CheckCircle2 className={`w-4 h-4 mt-0.5 ${accent === 'orange' ? 'text-orange-300' : 'text-emerald-300'}`} /> <span>{p}</span></li>
        ))}
      </ul>
    </div>
  )
...
function HormoziStrip() {
  return (
    <section className="py-14 bg-slate-900/80 border-t border-white/10">
    ...
    <section className="py-16 bg-slate-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-10 items-start">
        <div>
          ...
          <ul className="divide-y divide-white/10 rounded-2xl bg-slate-900/30">
            {bullets.map(([h, _], i)=>(
              <li key={i} className="p-4 text-white/80">{h}</li>
            ))}
          </ul>
        </div>
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-white heading mb-4">Soluția</h2>
          <ul className="divide-y divide-emerald-400/20 rounded-2xl bg-slate-900/20">
            {bullets.map(([_, s], i)=>(
              <li key={i} className="p-4 text-white/80">{s}</li>
            ))}
          </ul>
        </div>
      </div>
...
function Landing() {
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden hero-gradient">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative">
            ...
            <div className="rounded-2xl p-4 shadow-2xl bg-slate-900/40 border border-white/10">
              <div className="rounded-xl p-4 bg-slate-900/30 border border-white/10">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="rounded-xl p-3 bg-slate-900/20 border border-white/10">
      ...
      {/* Outcomes section */}
      <section className="py-16 bg-slate-900/80 border-t border-white/10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-emerald-600/20 border border-emerald-400/40 flex items-center justify-center text-emerald-300 text-xs">1</div>
              <div>
            ...
            <div className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-blue-600/20 border border-blue-400/40 flex items-center justify-center text-blue-300 text-xs">2</div>
              <div>
                <div className="flex items-center gap-3 mb-1 text-white">
            ...
            <div className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-orange-600/20 border border-orange-400/40 flex items-center justify-center text-orange-300 text-xs">3</div>
              <div>
                <div className="flex items-center gap-3 mb-1 text-white">
          ...
          <div>
            ...
            <ul className="grid sm:grid-cols-2 gap-2">
              {["Top Afaceri micro-întreprinderi Lungulețu (2024–2025)","Flotă proprie – camioane pentru marfă agricolă + generală","Experiență: mii de tone din Lungulețu către București","Programări la depozite, documente la un loc"].map((b, i) => (
                <li key={i} className="text-white/80 text-sm px-3 py-2 rounded-full bg-slate-900/30 border border-white/10">{b}</li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl p-5 bg-slate-900/40 border border-white/10">
            <div className="text-white/90 font-semibold mb-3">Comparativ – cum arată o zi de lucru</div>
            <div className="grid grid-cols-2 text-sm text-white/70 gap-6">
              <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {[
              ...
              <div key={idx} className="rounded-2xl p-5 bg-slate-900/30 border border-white/10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-white/80 text-sm">
            {[
              ...
              <div key={i} className="rounded-2xl bg-slate-900/30 border border-white/10 p-5">
